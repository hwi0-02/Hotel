...
public class EmailService {
    ...
    public String sendVerificationCode(String email) {
        try {
            // 이메일 존재 확인
            if (!loginRepository.existsByEmail(email.toLowerCase(Locale.ROOT).trim())) {
            ...
            // 코드 저장 (5분 유효)
            codeStorage.put(email.toLowerCase(Locale.ROOT).trim(), new VerificationCode(code, LocalDateTime.now().plusMinutes(5)));
    ...
    public void sendLoginLockNotification(String email, long minutes) {
        try {
            String subject = "[호텔 예약 플랫폼] 로그인 시도 제한 안내";
            String content = buildLockEmailContent(minutes);
            sendEmail(email, subject, content);
            log.info("로그인 잠금 안내 이메일 발송 완료: {} ({}분)", email, minutes);
        } catch (Exception e) {
            log.error("로그인 잠금 안내 이메일 발송 실패: {} - {}", email, e.getMessage());
        }
    }

    private String buildLockEmailContent(long minutes) {
        return "<div style='max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;'>" +
               "<h2 style='color: #333; text-align: center;'>로그인 시도 제한 안내</h2>" +
               "<div style='background-color: #f8f9fa; padding: 24px; border-radius: 12px;'>" +
               "<p style='color: #444; line-height: 1.6;'>" +
               "회원님의 계정에서 반복된 로그인 실패가 감지되어 보안을 위해 일시적으로 로그인 시도가 제한되었습니다." +
               "</p>" +
               String.format("<p style='color: #666; line-height: 1.6;'>제한 시간: <strong>%d분</strong><br>시간이 경과한 후 다시 로그인해 주세요.</p>", minutes) +
               "<p style='color: #888; font-size: 13px;'>본인이 시도한 내역이 아니라면 고객센터로 즉시 문의해 주세요.</p>" +
               "</div>" +
               "</div>";
    }
