...
public class AdminHotelService {
    ...
    public void approve(Long id, Long adminUserId, String note) {
        ...
        // 승인 시 소유자 사용자 권한을 BUSINESS로 승격
        Long ownerId = hotel.getUserId();
        if (ownerId != null) {
            userRepository.findById(ownerId).ifPresent(u -> {
                if (u.getRole() != User.Role.BUSINESS) {
                    u.setRole(User.Role.BUSINESS);
    ...
    // [CHANGED] 사업자번호 포맷: 선행 0 복원(10자리)
    private String businessNumberOf(Hotel hotel) {
        if (hotel == null || hotel.getBusinessId() == null) return null;
        long num = hotel.getBusinessId();
        // 왼쪽 0 패딩으로 10자리 보존 (DB가 숫자로 저장되어 선행 0이 사라진 경우 복원)
