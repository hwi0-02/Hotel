...
public class ReservationService {
    ...
    private RoomInventory getOrCreateLocked(Long roomId, LocalDate date) {
        return invRepo.findWithLock(roomId, date)
                .orElseGet(() -> {
                    // ★ 객실 보유 수로 기본 재고 설정(5 고정 제거)
                    int baseQty = roomRepo.findById(roomId)
                            .map(r -> {
                                Integer c = r.getRoomCount();
                                return (c != null && c > 0) ? c : 0;
                            })
                            .orElse(0);
                    return invRepo.save(RoomInventory.builder()
                            .roomId(roomId)
                            .date(date)
    ...
    public HoldResponse hold(HoldRequest req) {
        if (req.getUserId() == null)
            throw new IllegalArgumentException("userId is required (from JWT)");
        if (req.getQty() == null || req.getQty() < 1)
    ...
    private void cancelInternal(Reservation r) {
        ...
        // ★ PENDING/COMPLETED 모두 재고 복구
        boolean restore = (r.getStatus() == Reservation.Status.PENDING)
    ...
    /** 예약을 강제로 만료(PENDING→CANCELLED) + 재고 복구 */
    @Transactional
    public void expire(Long id) {
        ...
        resRepo.save(r);
        log.info("[EXPIRE] reservationId={} → EXPIRED (재고 복구 완료)", id);
    }

    @Transactional
    public Map<Long, Long> findOverlapLong(List<Long> roomIds, Instant startDate, Instant endDate) {
        List<Object[]> results = resRepo.sumNumRoomsOverlappingPeriod(roomIds, startDate, endDate, Instant.now());
        Map<Long, Long> out = new HashMap<>();
        for (Object[] row : results) {
            Long roomId = (Long) row[0];
            Long usedQty = ((Number) row[1]).longValue();
            out.put(roomId, usedQty);
        }
        return out;
    }
}
